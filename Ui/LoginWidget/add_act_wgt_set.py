...
def __save_btn_trough(
    refresh: callable, user_edit: QLineEdit, pwd_edit: QLineEdit, ssfn_edit: QLineEdit
) -> None:
    """保存按钮的槽函数"""
...
def __pwd_edit_toggles_visible_state(pwd_edit: QLineEdit):
    ...
    def judgement():
        ...
        if pwd_edit.echoMode() == QLineEdit.PasswordEchoOnEdit:
            ...
            action.setIcon(QIcon("./img/icon/LoginWidget/add_account/invisible.svg"))
        else:
...
def __account_save_file(user: str, pwd: str, ssfn: str = str()):
    ...
    __file_operation.modify_json(__file_operation.cammy_data_path, config, add=True)
